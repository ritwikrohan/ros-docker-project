...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<RepublishTFs-request>)))
  ...
  (cl:format cl:nil "## Notify the tf2_web_republisher that republishing of certain~%## TFs is desired~%string[] source_frames~%string target_frame~%float32 angular_thres~%float32 trans_thres~%float32 rate~%duration timeout # tf2_web_republisher will stop publishing the topic if it has zero subscribers for this amount of time~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'RepublishTFs-request)))
  "Returns full string definition for message of type 'RepublishTFs-request"
  (cl:format cl:nil "## Notify the tf2_web_republisher that republishing of certain~%## TFs is desired~%string[] source_frames~%string target_frame~%float32 angular_thres~%float32 trans_thres~%float32 rate~%duration timeout # tf2_web_republisher will stop publishing the topic if it has zero subscribers for this amount of time~%~%~%"))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<RepublishTFs-response>)))
  ...
  (cl:format cl:nil "# response~%string topic_name # a topic of type geometry_msgs/TransformStamped[] that publishes the requested transforms~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'RepublishTFs-response)))
  "Returns full string definition for message of type 'RepublishTFs-response"
  (cl:format cl:nil "# response~%string topic_name # a topic of type geometry_msgs/TransformStamped[] that publishes the requested transforms~%~%~%"))
